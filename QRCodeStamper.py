...
class QRCodeStamper():
    ...
    def stamp_qr_code(self, text, image, size, position) -> Image:
        ...
        for item_i, item_q in zip(image_coded.getdata(), qr_code.getdata()):
            if item_q[0] == 255:
                new_data.append((min(255,item_i[0]+self.gap_up), min(255,item_i[1]+self.gap_up), min(255,item_i[2]+self.gap_up), max(0,item_i[3]-self.gap_up)))
            else:
                new_data.append((max(0,item_i[0]-self.gap_down), max(0,item_i[1]-self.gap_down), max(0,item_i[2]-self.gap_down), min(255,item_i[3]+self.gap_down)))
